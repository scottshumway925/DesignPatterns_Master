...
void Pellet::output()
{
   if (!isDead())
   {
      vector<string> command = { "PELLET", to_string(pt.getX()), to_string(pt.getY()) };
      drawIt(command);
   }
}
...
void Bomb::output()
{
   if (!isDead())
   {
      vector<string> command = { "BOMB", to_string(pt.getX()), to_string(pt.getY()), to_string(radius)};
      drawIt(command);
   }
}
...
void Shrapnel::output()
{
    if (!isDead())
    {
       vector<string> command = { "SHRAPNEL", to_string(pt.getX()), to_string(pt.getY()), to_string(radius) };
       drawIt(command);
    }
}
...
void Missile::output()
{
    if (!isDead())
    {
        ...
        vector<string> command = { "MISSILE", to_string(pt.getX()), to_string(pt.getY()), to_string(ptNext.getX()), to_string(ptNext.getY()) };
        drawIt(command);
    }
}
