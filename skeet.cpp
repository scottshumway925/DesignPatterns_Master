...
void Skeet::drawBullseye(double angle) const
{
   ...
   Position center(x, y);
   Position left(x - 10.0, y);
   Position right(x + 10.0, y);

   Position bottom(x, y - 10.0);
   Position top(x, y + 10.0);

   gout.drawLine(left, right, 0.6, 0.6, 0.6);
   gout.drawLine(bottom, top, 0.6, 0.6, 0.6);

   Position aim(dimensions.getX(), 0.0);

   gout.drawLine(aim, center, 0.2, 0.2, 0.2);
...
void Skeet::drawStatus() const
{
   ...
   else
   {
      ...
      gout.drawHourglass(1.0 - time.percentLeft(),
